...
class AIService:
    ...
    def generate_analysis(self, report, mode: str = 'fast'):
        ...
        user_message = (
            f"ЗАКОН:\n{law_context}\n\n"
            f"ДОГОВОР:\n{document_text}\n\n"
            f"Проанализируй договор по закону и верни JSON по схеме:\n{schema_str}\n\n"
            f"ПРАВИЛА:\n"
            f"1. Если договор НЕ про жилищное право — верни {{\"findings\": []}}.\n"
            f"2. Не объединяй категории: 1 находка = 1 объект в findings.\n"
            f"3. 'explanation' — сухой и максимально простой пересказ суть для лайка.\n"
            f"4. 'source' — строго точный № статьи из ЗАКОНА. Если статьи нет в тексте закона выше — пиши 'Не найдено в предоставленном законе'. Не галлюцинируй статьи!\n"
            f"5. 'mitigation' — давай формулировку/совет, КАК исправить пункт (обязательно для 'risk' и завышенных сумм).\n\n"
            f"Выдай ТОЛЬКО валидный JSON: {{\"findings\": [...]}}"
        )
